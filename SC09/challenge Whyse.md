...
- In the Northwind database, what is the type of relationship between the
  `Employee` and `Territory` tables? "The 'RegionID' describes the general area
  from where the employees operate."
- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate? "One of the most useful times to
  incorporate a doc store would be when many different variables apply to one factor;
  when there is much shifting or changing on a wide level, or one needs to rely on ACID,
  relational databases are the way to go."
- What is "NewSQL", and what is it trying to achieve? "'NewSQL' is an attempt to combine
the best of both database types. It is a relatively new system."
